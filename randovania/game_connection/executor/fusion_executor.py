...
class FusionConnectionException(Exception):
    pass


@dataclasses.dataclass()
class FusionSocketHolder(CommonSocketHolder):
    request_number: int
...
class FusionExecutor:
    _port = 2017
    _socket: FusionSocketHolder | None = None
    ...
    async def connect(self) -> str | None:
        ...
        try:
            ...
            self._socket = FusionSocketHolder(reader, writer, 1, 0)
            ...
            if int(api_version) != self._current_version:
                raise FusionConnectionException("API versions mismatch!")
    ...
    async def _parse_packet(self, packet_type: int) -> bytes | None:
        ...
        match packet_type:
            case PacketType.PACKET_MALFORMED:
                ...
                self.logger.warning("Fusion received a malformed packet. Disconnecting.")
                raise FusionConnectionException
    ...
    async def _check_header(self) -> None:
        ...
        if received_number[0] != self._socket.request_number:
            ...
            raise FusionConnectionException(f"Expected response {self._socket.request_number}, got {num_as_string}")
